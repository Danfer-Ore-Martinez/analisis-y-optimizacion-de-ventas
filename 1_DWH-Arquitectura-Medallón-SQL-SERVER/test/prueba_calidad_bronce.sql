/*
===========================================================
VERIFICACIÓN DE LA CALIDAD DE INFORMACION EN LA CAPA BRONCE 
===========================================================

Proposito del Script:
	- Revisar tabla por tabla y columna por columna la calidad de la información, modificar valores, normalizar,
	estandarizar la información y ver que modificaciones se tiene que realizar en la capa plata, para luego instar la informacion limpia
  en la capa oro, se verifica:
...
/*========================================
VERIFICACIÓN CALIDAD "bronce.crm_cust_info"
========================================*/
SELECT TOP 1000 * FROM bronce.crm_cust_info;
...
-- Regresa valores-> Columna a limpiar
SELECT 
	cst_firstname
FROM bronce.crm_cust_info
...
-- RESULTADOS ESPERADO "S" o "M"

SELECT DISTINCT
	cst_marital_status
FROM bronce.crm_cust_info;

-- RESULTADOS ESPERADO "F" o "M"
SELECT DISTINCT
	cst_gndr
FROM bronce.crm_cust_info;
...
/*========================================
VERIFICACIÓN CALIDAD "bronce.crm_prd_info"
========================================*/
SELECT TOP 1000 * FROM bronce.crm_prd_info;
...
-- Verificar la logica de las fechas, y valores de fecha no extremos 
-- Resultados Esperados: Tabla sin resultados
SELECT
	prd_key,
	prd_start_dt,
...
SELECT 
	prd_start_dt
FROM bronce.crm_prd_info
WHERE  prd_start_dt >= '2050-01-01'
	  OR  prd_start_dt <= '1900-01-01';

SELECT 
	prd_end_dt
FROM bronce.crm_prd_info
WHERE  prd_end_dt >= '2050-01-01'
	  OR  prd_end_dt <= '1900-01-01';

/*=============================================
VERIFICACIÓN CALIDAD "bronce.crm_sales_details"
=============================================*/
SELECT TOP 1000 * FROM bronce.crm_sales_details;
SELECT TOP 1000 * FROM bronce.erp_px_cat_g1v2;
...
-- Verificar si las fechas son validas (fechas negativas o diferentes de cero, que la cantidad de caractares 
-- sea diferente de 8, que no tenga fechas extremas(mayor a 2050 o menor a 1900), logica en fechas orde_dt < due_dt y ship_dt)
...
/*=============================================
VERIFICACIÓN CALIDAD "bronce.erp_cust_az_12"
=============================================*/
SELECT TOP 1000 * FROM bronce.erp_cust_az12;
...
FROM bronce.erp_cust_az12
WHERE CID != TRIM(CID)
-- Verificación que todos los id se encuentren en bronce.crm_cust_info
-- Resultados Esperados: Tabla vacia
...
WHERE CASE
		WHEN cid LIKE 'NAS%'THEN SUBSTRING(cid,4,LEN(cid))
		ELSE cid
	END NOT IN (SELECT DISTINCT
					cst_key
				FROM bronce.crm_cust_info);
-- Verificación fechas, casos extremos personas con fecha de nacimiento hace 100 años o en el futuro
-- Resultados Esperados: Tabla vacia
SELECT 
	bdate
FROM bronce.erp_cust_az12
...
/*=============================================
VERIFICACIÓN CALIDAD "bronce.erp_loc_a101"
=============================================*/
SELECT TOP 100 * FROM bronce.erp_loc_a101
...
-- Resultados Esperados: Tabla vacia
SELECT 
	CONCAT(SUBSTRING(cid,1,2),SUBSTRING(cid,4,LEN(cid))) AS cdi,
	cntry 
FROM bronce.erp_loc_a101 
WHERE CONCAT(SUBSTRING(cid,1,2),SUBSTRING(cid,4,LEN(cid))) NOT IN (SELECT 
																		cst_key
																	FROM plata.crm_cust_info)
...
-- Resultados Esperados: United States, Germany, Australia, Canada, France, n/a
SELECT DISTINCT
	cntry
FROM bronce.erp_loc_a101

/*=============================================
VERIFICACIÓN CALIDAD "bronce.erp_px_cat_g1v2"
=============================================*/
SELECT TOP 100 * FROM bronce.erp_px_cat_g1v2

-- Verifiación si todas las id son iguales a las categorias de los productos 
-- En este momento solo existe 1 categoría llamada CO_PD que no se encuentra con producto, pero es
-- información valida
SELECT 
	id
